...
import { Edit, Trash2, Move, Info, Clock, User, Target, Flag, AlignLeft, ArrowRight } from "lucide-react";
...
interface MobileContextMenuProps {
  ...
  onDelete: () => void;
  onMove: () => void; 
  initialView?: "menu" | "properties"; // New Prop
}
...
export default function MobileContextMenu({
  ...
  const [viewState, setViewState] = useState<MenuState>(initialView);

  // Reset view state when initialView prop changes
  useEffect(() => {
  ...
  if (viewState === "menu") {
    return (
      <div className="space-y-2">
        {/* Header Preview */}
        <div className="flex items-center gap-3 p-3 bg-white/5 rounded-xl border-r-4 mb-4" style={{ borderRightColor: borderColor }}>
          ...
          {canModify && (
            <>
              <button onClick={onEdit} className="flex items-center gap-3 w-full p-3 rounded-xl bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 transition-colors">
                <Edit size={18} /> <span className="font-medium">ویرایش</span>
              </button>
              <button onClick={onMove} className="flex items-center gap-3 w-full p-3 rounded-xl bg-emerald-600/10 hover:bg-emerald-600/20 text-emerald-400 transition-colors">
                <Move size={18} /> <span className="font-medium">جابجایی (Drag)</span>
              </button>
              <button onClick={onDelete} className="flex items-center gap-3 w-full p-3 rounded-xl bg-red-600/10 hover:bg-red-600/20 text-red-400 transition-colors">
                <Trash2 size={18} /> <span className="font-medium">حذف</span>
              </button>
            </>
          )}
  ...
  return (
    <div className="space-y-4 animate-in slide-in-from-right-10 duration-200">
      <button onClick={() => setViewState("menu")} className="flex items-center gap-2 text-sm text-gray-400 hover:text-white mb-2">
