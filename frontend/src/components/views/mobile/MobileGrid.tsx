                ...
                {days.map((day, i) => {
                    ...
                    return (
                        <div key={i} className="flex-1 relative border-r border-white/10 h-full group">
                            ...
                            {/* Waving Placeholder for Draft */}
                            {draftEvent && draftEvent.date.toDateString() === day.toDateString() && (
                                <div
                                    className="absolute z-20 left-1 right-1 rounded-lg border-2 border-dashed border-emerald-500 bg-emerald-500/10 flex items-center justify-center animate-bounce shadow-[0_0_15px_rgba(16,185,129,0.4)]"
                                    style={{
                                        top: `${(draftEvent.startHour / 24) * 100}%`,
                                        height: `${((draftEvent.endHour - draftEvent.startHour) / 24) * 100}%`,
                                        animationDuration: '2s' // Slower bounce for "Waving" effect
                                    }}
                                >
