...
export default function SmartChart({ 
    ...
    const handleExport = useCallback(() => {
        ...
        setIsExporting(true);
        setTimeout(() => {
            toPng(ref.current!, { cacheBust: true, backgroundColor: '#18181b', style: { direction: 'rtl' } })
                .then((dataUrl) => {
    ...
    const CustomTooltip = ({ active, payload, label }: any) => {
        if (active && payload && payload.length) {
            return (
                <div className="bg-[#18181b] border border-white/10 p-3 rounded-xl shadow-2xl backdrop-blur-md z-50 text-right dir-rtl">
                    ...
                    <div className="flex items-center gap-2 justify-end">
                        ...
                        <span className="w-2 h-2 rounded-full shadow-[0_0_8px]" style={{ backgroundColor: color, boxShadow: `0 0 10px ${color}` }}></span>
                    </div>
                </div>
            );
        }
            ...
            <div className="p-4 border-b border-white/5 bg-white/5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <div className="flex bg-black/40 rounded-lg p-0.5 border border-white/5">
                        <button onClick={() => setRange('24h')} className={clsx("px-2 py-1 text-[10px] font-bold rounded-md transition-all", range === '24h' ? "bg-white/10 text-white shadow-sm" : "text-gray-500 hover:text-gray-300")}>۲۴س</button>
                        <button onClick={() => setRange('7d')} className={clsx("px-2 py-1 text-[10px] font-bold rounded-md transition-all", range === '7d' ? "bg-white/10 text-white shadow-sm" : "text-gray-500 hover:text-gray-300")}>۷ر</button>
                        <button onClick={() => setRange('30d')} className={clsx("px-2 py-1 text-[10px] font-bold rounded-md transition-all", range === '30d' ? "bg-white/10 text-white shadow-sm" : "text-gray-500 hover:text-gray-300")}>۳۰ر</button>
                    </div>
                    <button onClick={handleExport} disabled={isExporting} className="p-1.5 hover:bg-white/10 rounded-lg text-gray-400 transition-colors" title="ذخیره تصویر">
                        {isExporting ? <Loader2 size={16} className="animate-spin text-blue-400" /> : <Camera size={16}/>}
                    </button>
                </div>
            </div>

            <div className="p-4 w-full relative dir-ltr" style={{ height }}>
                {filteredData.length > 0 ? (
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-600">
                        ...
                        <span className="text-xs">داده‌ای موجود نیست</span>
                    </div>
                )}
            </div>
        </motion.div>
