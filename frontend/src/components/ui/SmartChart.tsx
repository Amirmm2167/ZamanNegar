...
export default function SmartChart({ 
    ...
    const filteredData = useMemo(() => {
        ...
        // In a real app, you'd parse dates. Here we slice array for demo simplicity
        // assuming data is sorted.
        const len = data.length;
        if (range === '24h') return data.slice(Math.max(len - 24, 0));
        if (range === '7d') return data.slice(Math.max(len - 7, 0));
        ...
        >
            ...
            <div className="p-4 border-b border-white/5 bg-white/5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    {Icon && (
                        <div className="p-2 bg-[#000000]/40 rounded-lg text-gray-300 border border-white/5">
                            <Icon size={16} />
                        </div>
                    )}
                        ...
                        {['24h', '7d', '30d'].map((r) => (
                            <button
                                ...
                                className={clsx(
                                    ...
                                    range === r ? "bg-white/10 text-white shadow-sm" : "text-gray-500 hover:text-gray-300"
                                )}
                            >
                                {r.toUpperCase()}
                            </button>
